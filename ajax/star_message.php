...
$existsStmt = mysqli_prepare($conn, "SELECT id FROM starred_messages WHERE message_id = ? AND user_id = ? LIMIT 1");
...
if ($action === 'remove' || ($action === 'toggle' && $exists)) {
    $stmt = mysqli_prepare($conn, "DELETE FROM starred_messages WHERE message_id = ? AND user_id = ?");
    ...
    $stmt = mysqli_prepare($conn, "
        INSERT IGNORE INTO starred_messages (message_id, user_id, created_at)
        VALUES (?, ?, NOW())
    ");
    mysqli_stmt_bind_param($stmt, "ii", $message_id, $user_id);
    $ok = mysqli_stmt_execute($stmt);
